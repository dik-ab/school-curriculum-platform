...
このページでは、Dockerを触り始める前に「コンテナ（Container）とは何か」という概念を理解します。前のセクション（[バックエンド基礎（NestJS）](/backend//)）では、自分のPCに直接Node.jsをインストールしてメモAPIを動かしました。その方法で何が困るのか、コンテナがそれをどう解決するのかを、仮想マシン（VM）との比較を交えて見ていきます。
...
あなたは[CRUD実践](/backend/crud_practice/)でメモAPIを作りました。これを友人のPCや、本番のサーバーで動かすことを考えてみます。アプリが動くためには、コード以外にもたくさんの「前提条件」が必要です。
...
とはいえ、私たちが作るWeb API・データベース・Webサーバーは、いずれもコンテナが最も得意とする領域です。安心して先へ進みましょう。
