...
実際にProject 01のM1を通しで検証したときに踏んだ、環境起因のつまずきです。仕様の理解とは無関係に時間を溶かすポイントなので、先に知っておいてください(詳細な対処は各仕様リポジトリの `docs/tips.md` にあります)。

| 症状 | 原因と対処 |
| --- | --- |
| pnpmでPrisma等が動かない | pnpm 10はビルドスクリプトを既定でブロックする。`package.json` の `pnpm.onlyBuiltDependencies` で許可する |
| Prismaの起動・ビルドが壊れる | Prisma 7の破壊的変更(driver adapter必須、出力ESM化、seedの登録場所)。`docs/tips.md` の手順どおりに設定する |
| Apple SiliconでMailHogが起動しない | amd64専用イメージのため `platform: linux/amd64` を指定する |
| ポート5432や5173が使えない | 他プロジェクトと衝突。ホスト側ポートは環境変数で変更してよい(仕様が固定しているのはコンテナ側) |
| メール確認が「無効なトークン」になる | React StrictModeがeffectを二重実行する。トークンを消費するAPIは1回だけ呼ぶガードを入れる |
| ログアウトしてもCookieが残る | `clearCookie` は発行時と同じ属性(Path/SameSite等)を指定しないと消えない |
