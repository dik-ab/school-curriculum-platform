...
配列の番号（インデックス）は `0` から始まります。`{80, 90, 70}` は次のように並んでいるイメージです。

| インデックス | `0` | `1` | `2` |
| --- | --- | --- | --- |
| 値 | 80 | 90 | 70 |

要素は3つですが、インデックスは `0`・`1`・`2` で、いちばん大きい番号は「要素数 − 1」です。存在しない `scores[3]` にアクセスすると `ArrayIndexOutOfBoundsException`（範囲外エラー）になります。
