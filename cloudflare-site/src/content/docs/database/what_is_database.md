...
このように「ユーザー1人に対して投稿が複数ある」関係を**1対多（one-to-many）**と呼びます。リレーションの詳細は[リレーション](/database/relations/)のページで実際にコードを書きながら学びますが、「外部キーでテーブル同士をつなぐ」という基本イメージをここで持っておいてください。
...
それぞれの文法を、`users` テーブルを例に見てみましょう。実際に手を動かすのは[次のページ](/database/postgresql_setup/)で行うので、ここでは「読んで意味が分かる」ことを目指してください。

### SELECT — データを取得する

```sql
-- usersテーブルの全行・全列を取得する
SELECT * FROM users;

-- 列を指定して取得する（nameとemailだけ）
...
- `SELECT *` — `*`（アスタリスク）は「すべての列」という意味です
- `FROM users` — どのテーブルから取得するかを指定します
- `WHERE id = 1` — `WHERE` 句で「条件に合う行だけ」に絞り込みます。TypeScriptの `filter` に近い役割です
- `--` から行末まではコメントです。文末のセミコロン `;` はSQL文の区切りを表します

### INSERT — データを追加する

```sql
-- usersテーブルに新しい行を追加する
INSERT INTO users (name, email) VALUES ('三郎', 'saburo@example.com');
```

**コード解説**

- `INSERT INTO users (name, email)` — `users` テーブルの `name` 列と `email` 列に値を入れる、という宣言です
- `VALUES ('三郎', 'saburo@example.com')` — 入れる値を、列の並び順と対応させて書きます
- SQLでは文字列を**シングルクォート** `'...'` で囲みます（ダブルクォートは別の意味になるので注意）
- `id` を指定していないのは、自動採番に任せるためです
...
- `UPDATE users` — 更新対象のテーブルを指定します
- `SET name = '太郎丸'` — どの列をどの値に変えるかを指定します
- `WHERE id = 1` — **どの行を**更新するかの条件です。**`WHERE` を忘れると全行が更新されてしまう**ため、UPDATEでは特に注意が必要です
...
- `DELETE FROM users` — 削除対象のテーブルを指定します
- `WHERE id = 3` — 削除する行の条件です。こちらも **`WHERE` を忘れると全行が消えます**。SQLで最も恐れられている事故の1つです
...
- アプリケーションとデータベースの間の言語は**SQL**です。ただし、TypeScriptのコードの中でSQLを文字列として組み立てるのは大変でミスも起きやすいため、実際の開発では**ORM**という道具を使ってTypeScriptの関数呼び出しでデータベースを操作します。これが[Prisma](/database/prisma_setup/)です

つまりこのセクションの学習は、「まず生のSQLでデータベースの動きを理解し（次ページ）、そのうえでPrismaに任せる（それ以降）」という二段構えになっています。SQLを知らずにORMだけ使うと、トラブル時に何が起きているか分からなくなるため、遠回りに見えてもSQLから学びます。
