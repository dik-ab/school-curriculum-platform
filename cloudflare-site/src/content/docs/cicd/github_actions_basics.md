...
- `version: 9` — インストールするpnpmのバージョンです。手元で使っているpnpmとメジャーバージョンを揃えます
...
- `      - name: Setup pnpm` / `uses: pnpm/action-setup@v4` — ランナーにpnpm（バージョン9）をインストールします。次のステップの `cache: 'pnpm'` より前に必要です
