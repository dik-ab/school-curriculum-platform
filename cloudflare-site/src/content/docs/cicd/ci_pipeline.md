...
- `pnpm exec jest --config ./test/jest-e2e.json` — `pnpm run test:e2e` ではなくjestを直接起動している点に注意してください。`test:e2e` スクリプトは `dotenv -e .env.test` で接続情報を読み込みますが、`.env.test` は `.gitignore` に入れておりCIのランナー上には存在しないため、そのままでは動きません
- `env: DATABASE_URL: ...` — その代わりに、各ステップへ環境変数として接続文字列を直接渡します。[Prismaのセットアップ](/database/prisma_setup/)で `.env` に書いていたものを、CIではステップの `env` で渡す、という対応関係です。同じ構成は[SNS開発のテスト](/sns/nestjs/testing/)でも実際に組みます
