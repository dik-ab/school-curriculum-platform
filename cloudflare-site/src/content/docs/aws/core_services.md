...
2. ユーザーがアップロードするプロフィール画像などの保存先（→ SNS開発の[プロフィールと画像アップロード](/sns/nestjs/profile_and_images/)）
...
**SNSアプリでの役割:** 会員登録時の**確認メール**の送信です（→ SNS開発の[メールアドレス確認](/sns/nestjs/email_verification/)）。
