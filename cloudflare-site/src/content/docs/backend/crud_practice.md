...
description: NestJSでメモAPIの作成・取得・更新・削除を実装し、curlで全エンドポイントを動作確認するセクションの集大成
...
このメモAPIは今後のセクションの共通題材です。[Docker基礎](/docker/)ではコンテナ技術そのものを学び、[データベースとPrisma](/database/)ではこのAPIのデータを永続化し、[バックエンドテスト](/testing/)ではテストを書きます。プロジェクトは消さずに保管しておいてください。
