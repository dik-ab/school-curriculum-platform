...
`@IsEmail()`は、SNS開発セクションの[認証](/sns/auth/)でユーザー登録を作るときに活躍します。
...
DTOとバリデーションは、この後すべてのAPI開発で使います。SNS開発セクションでは、ユーザー登録の入力検証（[認証](/sns/auth/)）や投稿の文字数制限（[投稿機能](/sns/posts/)）として再登場します。
