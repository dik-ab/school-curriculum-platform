...
ここまで完走したことを、まず静かに振り返ってみてください。手元には、ユーザー登録からメール確認、投稿、いいね、フォロー、リアルタイムのDM、画像アップロードまでを備え、テストに守られ、[AWSの本番環境](/sns/deploy/)にデプロイされたSNSアプリがあります。半年前、変数の型から学び始めたときには遠く見えたはずのものです。
...
### 環境構築（[プロジェクトセットアップ](/sns/project_setup/)）
...
### メール確認（[メールアドレス確認](/sns/email_verification/)）
...
### フォロー（[フォローとフォロー中タイムライン](/sns/follow/)）
...
### プロフィールと画像（[プロフィール編集と画像アップロード](/sns/profile_and_images/)）
...
- **画像つき投稿** … アバターで作ったpresigned URLの仕組みを投稿画像に応用する（→ [プロフィール編集と画像アップロード](/sns/profile_and_images/)）。1投稿に複数枚を許すならテーブルが1つ増えます
...
- **構成図** … [デプロイ](/sns/deploy/)で描いたアーキテクチャ図（Mermaidで描けばGitHub上で描画されます）とER図
...
常時AWSで動かし続けると課金が続いてしまうので、現実的な見せ方を選びます。画面収録（GIFや短い動画）をREADMEに載せるのが手軽で効果的です。面接などの予定に合わせて `cdk deploy` で一時的に本番を立ち上げ、終わったら `cdk destroy` する、という運用も[デプロイ](/sns/deploy/)を完了したあなたなら数十分でできます。
...
前のページは[AWSへの全体デプロイ](/sns/deploy/)です。追加課題を実装したら、ぜひもう一度デプロイまで通してみてください。機能追加→テスト→push→自動デプロイ、という流れを自分の意思で回せたとき、この章で作った仕組みが本当に自分のものになります。

そして——このページをもって、本カリキュラムのすべての課程は修了です。入門編の最初のページでターミナルの開き方から始めた学習が、テストとCI/CDに守られたフルスタックアプリをAWSで運用するところまで到達しました。ここから先は、決められたカリキュラムのない、自分で道を選ぶ学習です。作りたいものを見つけて、小さく作り、動かして、確かめる。この半年間やってきたことを、これからも続けてください。完走、おめでとうございます。
