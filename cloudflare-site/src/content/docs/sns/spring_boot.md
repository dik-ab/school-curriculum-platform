...
現在の解答コードは、MavenベースのSpring Boot APIとしてローカルで動く範囲に絞っています。DBはDocker ComposeのPostgreSQL、APIは `./mvnw spring-boot:run`、Reactは共通フロントを使います。ログイン成功時は `sns_session` HttpOnly Cookieを発行し、各Controllerで `AuthService` から現在のユーザーを解決します。実装上はCookieを優先し、curlや互換確認用の予備経路として `Authorization: Bearer` も受け取れます。
...
- セッションは `session_tokens` に保存しますが、現時点ではトークンハッシュ化と有効期限カラムまでは実装していません。Cookieの値はランダムなセッショントークンで、JWTではありません。
