...
description: Reactフロントエンドを共通にし、NestJSやRailsなど6つのバックエンドスタックで同じSNSを作る最終プロジェクトの入口
nav_order: 19
has_children: true
permalink: /sns/
...
いずれかのスタックでSNSを完成させたら、[実務プロジェクト応用編](/advanced/)に進んでください。要件定義書とGitHub Issuesだけを渡されて、AIと一緒に実務と同じ流れで開発する応用フェーズです。カリキュラムで学んだ知識を、指示のない状態で使いこなす訓練になります。
