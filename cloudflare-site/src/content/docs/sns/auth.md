...
- フロントエンドでトークンを保存し、APIリクエストに自動で付与する仕組みを作れる
...
sequenceDiagram
    ...
    C->>C: トークンを localStorage に保存

    Note over C,D: 3. 認証が必要なリクエスト
    C->>A: GET /auth/me（Authorization: Bearer トークン）
    A->>A: Guard が署名と有効期限を検証
...
- ログイン成功時にAPIがJWTを発行し、以降クライアントは毎回のリクエストの `Authorization` ヘッダ（オーソリゼーションヘッダ。認証情報を運ぶHTTPヘッダ）にトークンを添えます。
...
`GET /auth/me` はGuardを作ってから追加します。まずここまでをcurlで確認しましょう。バックエンドを起動して（`pnpm run start:dev`）、別のターミナルから実行します。
...
```json
{"accessToken":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOjEsInVzZXJuYW1lIjoiYWxpY2UiLCJpYXQiOjE3NjU1MzYwMDAsImV4cCI6MTc2NTYyMjQwMH0.dBjftJeZ4CVP-mB92K27uhbUJU1p1r_wW1gFWFOEjXk"}
```

JWTが返ってきました。パスワードを間違えると401が返ることも確認してください。
...
SNSのAPIの大半（投稿、いいね、フォローなど）は「ログイン済みユーザーだけが使える」ものです。すべてのハンドラの先頭に「トークンを検証して、無効なら401を返す」コードをコピペするのは現実的ではありません。NestJSはこの「リクエストがハンドラに届く前の関門」として、Guardという仕組みを用意しています。
...
- `extractToken(request)` — `Authorization` ヘッダは `Bearer eyJhbGci...` という形式（Bearer、ベアラー＝「持参人」。トークンを持っている人を信用する方式の名前）なので、空白で分割して、1語目が `Bearer` のときだけ2語目のトークンを返します。ヘッダ自体がない場合は `?.` と `?? []` の組み合わせで `undefined` になります。
...
トークンなしでアクセスすると、Guardに止められて401が返ります。
...
実行結果の例（`eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOjEs...` のようなJWTが表示されます）。トークンを付けて再アクセスします。
...
トークンなし→401、トークンあり→200。Guardが期待通りに働いています。バックエンドの認証はこれで完成です。

## フロントエンドの実装

次はReact側です。登録画面とログイン画面を作り、取得したトークンを保存して、以降のAPIリクエストに自動で付与する仕組みを作ります。
...
[fetchでAPI通信](/react/api_fetch/)では、画面ごとに `fetch` を直接書きました。しかしこのプロジェクトでは「全リクエストにトークンを付ける」「401なら全画面共通でログイン画面へ飛ばす」といった共通処理が必要です。そこで `fetch` を包む共通関数（HTTPクライアントライブラリでは「インターセプタ」と呼ばれる仕組みに相当する、自作のラッパー）を1つ作り、以後すべてのAPI呼び出しをこれに集約します。
...
- `saveToken` / `clearToken` / `isLoggedIn` — トークンの保存・削除・有無確認です。保存先は `localStorage`（ローカルストレージ。ブラウザにキーと値を永続保存する仕組み）で、ブラウザを閉じても消えません。キー名は `"token"` で統一します。
...
> **注意: localStorageとXSS**
>
> トークンを `localStorage` に保存する方式には、知っておくべきリスクがあります。ページ内でJavaScriptが実行できれば `localStorage` は読めるため、もしXSS（クロスサイトスクリプティング。攻撃者のスクリプトをページに注入される攻撃）が成立すると、トークンが盗まれる可能性があります。実務では `HttpOnly` 属性付きCookie（JavaScriptから読めないCookie）を使う設計も広く採用されます。
>
> このプロジェクトでは、(1) Reactは値の描画時に自動でエスケープするためXSSが起きにくいこと、(2) トークンの流れが目に見えて学習に適していること、(3) 有効期限を1日に絞っていることから、localStorage方式を採用します。「リスクを理解した上で選んでいる」ことが重要です。
...
- `apiFetch<{ accessToken: string }>` — レスポンスの型を指定しているので、`res.accessToken` が型補完されます。
- `saveToken(res.accessToken)` — トークンをlocalStorageに保存します。以後の `apiFetch` は自動でこのトークンを付与します。
- `navigate("/")` — ログイン成功でトップページ（タイムライン予定地）へ遷移します。
...
import { apiFetch, clearToken, isLoggedIn } from "./lib/apiClient";
...
- `useEffect` のリダイレクト — 未ログイン（トークンなし）で `#/login`・`#/register` 以外を開いたら、ログイン画面へ強制遷移します。描画のたびに副作用として判定したいので `useEffect` に書きます。
- `if (path === ...) return <...>` — ルーティングの本体です。条件付きレンダリングだけで実現できるシンプルさが、ハッシュルーティングの利点です。
- `TemporaryHome` — ログイン後の仮置きページです。マウント時に `GET /auth/me` を呼んで表示名を出すことで、「トークンが正しく送られている」ことを画面でも確認できます。[投稿機能とタイムライン](/sns/posts/)で本物のタイムラインに置き換えます。
- `handleLogout` — トークン方式のログアウトは「クライアントがトークンを捨てる」だけです。サーバーに状態がないので、サーバー側の処理は不要です。
...
3. **ログイン**: 登録した情報でログインすると、「ようこそ、アリス さん」のような表示に変わる。開発者ツール（Application → Local Storage）を開くと、`token` キーにJWTが保存されている。
...
**Q5. トークン方式の「ログアウト」で、サーバー側の処理が不要なのはなぜですか。**

<details markdown="1">
<summary>解答を見る</summary>

JWTによるトークン方式では、ログイン状態はサーバーではなく**クライアントが持つトークン**として表現されています。サーバーはトークンを記録しておらず、毎回署名を検証しているだけです。そのため、クライアントがlocalStorageからトークンを削除すれば、もう認証付きリクエストを送れなくなり、実質的にログアウトが完了します。

裏を返すと、発行済みトークンを有効期限前にサーバー側から無効化することは（ブロックリストなどの追加の仕組みなしには）できません。これがトークン方式の弱点であり、有効期限を短め（このプロジェクトでは1日）に設定する理由でもあります。
...
- [ ] `apiFetch` がトークンの自動付与・401処理・エラーメッセージ抽出をどう行っているか説明できる
- [ ] 登録→ログイン→`/auth/me`の流れを、curlだけで再現できる
- [ ] localStorageにトークンを置くことのリスク（XSS）と、このプロジェクトでの採用理由を説明できる
