...
CIのワークフロー（`ci.yml`）は、[CIパイプラインを作る](/cicd/ci_pipeline/)の手順で作成済みのものをこのリポジトリでもそのまま使います（まだ作っていない場合は、同ページの手順でいつでも追加できます）。デプロイ用のワークフローは[AWSへの全体デプロイ](/sns/nestjs/deploy/)で作成します。
...
ファイル構成は[NestJSのプロジェクト作成](/backend/setup/)で見たものと同じです。雛形のサンプル実装のうち、サンプルのテストファイルと、`Hello World!` を返すだけのAppServiceは使わないので削除します（テストは[SNSのテストを書く](/sns/nestjs/testing/)で改めて書きます）。
...
- `JWT_SECRET` — 次のページ[ユーザー登録とログイン（JWT認証）](/sns/nestjs/auth/)で使う、認証用の秘密の文字列です。今は仮の値で構いません（本番では推測不可能な長い値にし、[Secrets Manager](/aws/rds/)で管理します）
...
**ここではまだモデル（テーブル定義）を書きません。** 最初のモデル `User` は、次のページ[ユーザー登録とログイン（JWT認証）](/sns/nestjs/auth/)で追加し、最初のマイグレーションを実行します。今はPrisma Clientを一度生成しておきます（モデルが空でも生成は成功し、後でNestJSからimportできるようになります）。
...
2. **後**: 本番環境でALB（ロードバランサー）が「このコンテナにリクエストを送ってよいか」を定期的に確認するために使う（→ [ECR + ECS Fargate](/aws/ecr_ecs/)で学んだALBのヘルスチェックです。設定は[AWSへの全体デプロイ](/sns/nestjs/deploy/)で行います）
...
- この仮実装は、[ユーザー登録とログイン（JWT認証)](/sns/nestjs/auth/)で「URLのハッシュを見てページを出し分ける」本実装に置き換わります
...
ブラウザの同一オリジンポリシーによってブロックされ、開発者ツールにCORSエラーが表示されます。フロントエンドは `http://localhost:5173`、APIは `http://localhost:3000` で**ポートが違う＝別オリジン**だからです。`enableCors` を設定すると、APIが応答に `Access-Control-Allow-Origin` ヘッダを付け、指定したオリジンからの利用をブラウザに許可させられます（→ [HTTPとREST](/backend/http_and_rest/)、[つなぎ込みで起きること](/fullstack-todo/nestjs/integration/)）。
...
今は「NestJSが起動し、リクエストに応答できる状態か」をcurl一発で確認する手段です。後の[AWSへの全体デプロイ](/sns/nestjs/deploy/)では、ALB（ロードバランサー）がこのエンドポイントを定期的に呼び、応答しないコンテナをリクエストの振り分け先から外す**ヘルスチェック**として使います（→ [ECR + ECS Fargate](/aws/ecr_ecs/)）。
...
次のページは[ユーザー登録とログイン（JWT認証）](/sns/nestjs/auth/)です。最初のモデル `User` をPrismaスキーマに追加してマイグレーションを実行し、ユーザー登録・ログインのAPIと画面を作ります。このセクションで唯一の新しい概念である**JWT・bcrypt・Guard**が登場する、山場の1つです。このページで整えたValidationPipe（DTOの検証）、CORS、PrismaService、`JWT_SECRET` は、すべてそこで早速使われます。
