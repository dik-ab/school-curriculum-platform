...
- HTTPのGuardがWebSocket接続にはそのまま効かない理由を説明し、handshakeで届くCookieのJWTを自前で検証できる
...
ConversationがUserに**2本の線**でつながっている点は、[フォロー](/sns/nestjs/follow/)のFollowテーブルと同じ構図です（会話も「ユーザーとユーザーの関係」だからです）。したがってここでも名前付きリレーション（`"UserOne"` / `"UserTwo"`）が必要になります。
...
- `@relation("UserOne")` / `@relation("UserTwo")` — ConversationとUserの間には「1人目として参加」「2人目として参加」の2本の線があるため、[フォローのページ](/sns/nestjs/follow/)で学んだ名前付きリレーションで区別します。
...
sequenceDiagram
    ...
    R->>G: 接続（withCredentialsでCookie送信）
    G->>G: sns_sessionのJWTを検証 → client.data.user に保存<br>（失敗したら切断）
...
- `findOrCreate` — 「会話を開始する」操作です。すでに会話があればそれを、なければ新規作成して返します。
  - 相手をusernameで検索し、いなければ404、自分自身なら400。[フォロー](/sns/nestjs/follow/)の`follow`メソッドと同じチェックの並びです。
...
- `findMessages` — 履歴の取得です。会話が存在しなければ404、**自分が当事者でなければ403** `ForbiddenException`を返します。認証（誰であるか）は通っていても、他人の会話を覗くことは許可（authorization）されていない、という[投稿の削除](/sns/nestjs/posts/)で学んだ403の使い方です。履歴は`createdAt: 'asc'`（古い順）で返します。チャット画面は上から古い順に並べるためです。
...
- クラス全体を[JwtAuthGuard](/sns/nestjs/auth/)で保護し、`@CurrentUser()`でログイン中のユーザーIDを受け取る、おなじみの形です。
...
`@10` は、NestJS 10系プロジェクトとのpeer dependency不整合を防ぐためのメジャーバージョン固定です（→ [ユーザー登録とログイン（JWT認証）](/sns/nestjs/auth/)で説明した理由と同じです）。
...
### HTTPのGuardはWebSocket接続にはそのまま効かない

実装の前に、このページで一番大事な注意点を説明します。**[auth.md](/sns/nestjs/auth/)で作ったJwtAuthGuardは、WebSocketの接続開始そのものをそのまま守ってはくれません。**

理由は2つあります。

1. JwtAuthGuardはHTTPリクエスト用です。このSNSでは通常のAPI認証に `sns_session` というHttpOnly Cookieを使っていますが、WebSocketでは接続時の**handshake**（ハンドシェイク。→ [WebSocketの基礎](/realtime/websocket_basics/)）でCookieが届くため、HTTPのControllerに対するGuardとは別の場所で取り出す必要があります。
...
そこで、`handleConnection`でhandshakeのCookieから `sns_session` を取り出し、JWTを**自前で**`JwtService.verifyAsync`にかけます。検証に成功したらペイロードを`client.data.user`に保存し、失敗したら即切断します。`client.data`はSocket.IOが用意している「この接続に紐づく自由なメモ欄」で、HTTPでGuardが`request.user`に書き込んでいたのと同じ役割を果たします。一度検証すれば、**同じ接続から届く以後のイベントはすべてそのユーザーのもの**とみなせます。これがWebSocketで認証が「接続時に1回」で済む理由です。
...
- `cors: { origin: process.env.FRONTEND_URL, credentials: true }` — HTTPのCORS設定（[プロジェクトセットアップ](/sns/nestjs/project_setup/)の`enableCors`）はWebSocketのhandshakeには別途必要なので、Gateway側にも指定します。`credentials: true` を付けることで、React側が `withCredentials: true` で送る `sns_session` Cookieを受け取れます。
...
- `parseCookie` — Cookieヘッダの文字列を `{ sns_session: "..." }` のようなオブジェクトに変換する小さな関数です。HttpOnly CookieはブラウザのJavaScriptから読めませんが、サーバーには通常のCookieヘッダとして届きます。
...
- `handleSendMessage` — メッセージ送信です。
  - `payload.content?.trim()`と長さチェック — [プロジェクトセットアップ](/sns/nestjs/project_setup/)で設定したグローバルなValidationPipeは**HTTPリクエスト用**で、Gatewayのハンドラには自動では効きません。そのため最低限の検証（空でない・1000文字以内）を手で書いています。
...
WebSocketの前に、REST部分だけ先に確かめておきます。[投稿機能とタイムライン](/sns/nestjs/posts/) と同じ要領でaliceのCookie jar（`alice.cookies`）を用意した状態で、aliceからbobとの会話を開始します。
...
もう一度同じリクエストを送っても、**新しい会話は作られず同じid: 1が返る**ことを確認してください（find or create）。`GET /conversations`（一覧に上の会話が1件）と`GET /conversations/1/messages`（まだ`[]`）も同じ要領で確認できます。第三者（aliceでもbobでもないユーザー）のCookieで履歴を呼ぶと403になることも、余裕があれば確認してみてください。
...
- 1つ目のuseEffect（接続）
  - `socketUrl` — WebSocketの接続先です。ローカル開発では `VITE_API_URL` と同じ `http://localhost:3000` を使えますが、デプロイ時にAPIとSocket.IOの入口を分けたい場合に備えて `VITE_SOCKET_URL` も読めるようにしています。
  - `io(\`${socketUrl}/chat\`, { withCredentials: true })` — URL末尾の`/chat`がGatewayで指定したnamespaceです。`withCredentials: true` により、ログイン時に発行された `sns_session` CookieがWebSocketのhandshakeにも送られます。HttpOnly CookieなのでReactから値を読む必要はありません。
...
- 2つ目のuseEffect（初期データ）— 会話一覧と、自分のID（`GET /auth/me`。→ [認証のページ](/sns/nestjs/auth/)）をRESTで取得します。自分のIDは、メッセージを左右どちらに寄せるかの判定（`message.senderId === meId`）に使います。
...
他のページと同じく、ページ自身は`Layout`を含めず**App.tsx側で`Layout`に包みます**。ヘッダー（[投稿機能のページ](/sns/nestjs/posts/)で作ったLayout）の「チャット」リンクから`#/chat`へ遷移できるようになります。

## 動作確認 — 2つのブラウザでリアルタイムを体感する

DB・バックエンド・フロントエンドをすべて起動し、リアルタイム性を確認します。WebSocketの動作確認は**別ユーザーでログインした2つのブラウザ**を並べるのが基本です（同じブラウザの通常タブ同士ではCookieを共有するため、同時に別ユーザーとしてログインできません）。
...
ログアウトした状態、またはCookieがない別ブラウザで`#/chat`を開くと、接続が即座に切断されることも確認できます。`handleConnection`の自前検証が働いている証拠です。
...
(1) WebSocketの接続開始はHTTP Controllerのリクエスト処理とは別の流れだからです。このSNSではJWTを `sns_session` Cookieに入れていますが、Socket.IOでは接続時のhandshakeに含まれるCookieヘッダから取り出す必要があります。(2) Guardが介入できるタイミングの問題です。NestJSのGuardはハンドラ（HTTPルートや`@SubscribeMessage`）の手前で動く仕組みで、「接続が確立する瞬間」には介入できません。認証できない相手はイベントを処理する前に門前払いしたいので、`handleConnection`で自前検証し、失敗したら`client.disconnect()`します。検証に成功したペイロードは`client.data.user`に保存し、以後のイベントで「誰の接続か」として使います。
...
- [ ] handshakeのCookieから `sns_session` を取り出し、`verifyAsync`で検証し、失敗時に切断する`handleConnection`を写経せずに書ける
...
- 前のページ: [フォローとフォロー中タイムライン](/sns/nestjs/follow/)
- 次のページ: [プロフィール編集と画像アップロード](/sns/nestjs/profile_and_images/) — プロフィールの編集と、S3への画像アップロードでアバターを設定できるようにします。
- このページのChatGatewayは、[AWSへの全体デプロイ](/sns/nestjs/deploy/)でWebSocketをALB越しに動かすときの設定（アイドルタイムアウトなど）に関わってきます。
- 将来ECSタスクを2台以上に増やす場合は、Socket.IOの接続先固定とタスク間配信が必要になります。詳しくは[Socket.IOを複数台にするときのRedis](/sns/nestjs/deploy/#socketioを複数台にするときのredis)で扱います。
