...
`UsersController`（`backend/src/users/users.controller.ts`）にルートを追記します。認証には[ユーザー登録とログイン（JWT認証）](/sns/nestjs/auth/)で作った`JwtAuthGuard`と`@CurrentUser()`をそのまま使います。
...
- `user.sub` — JWTのペイロードに入っているユーザーIDです（→ [ユーザー登録とログイン（JWT認証）](/sns/nestjs/auth/)の`JwtPayload`）。URLやボディで「誰を更新するか」を受け取らず、**トークンから特定した本人だけ**を更新します。他人のプロフィールを書き換えられない構造です。

### curlで動作確認

[投稿機能とタイムライン](/sns/nestjs/posts/)までと同じ要領で、ログインしてトークンを取得してから試します。`$TOKEN`にはログインで得た`accessToken`が入っているものとします。
