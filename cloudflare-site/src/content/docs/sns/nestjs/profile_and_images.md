...
この出力は「どの権限が必要か」を確認するためのものです。なお、変数 `create_iam_user`(デフォルト `true`)を有効にしていると、このポリシーを持つ学習用の専用IAMユーザーとアクセスキーも作成されます。自分の既存IAMユーザーの認証情報をそのまま使う場合は `create_iam_user = false` にしてください。Terraformリポジトリは、あなたの既存IAMユーザーやECSタスクロールには権限を付けません。
