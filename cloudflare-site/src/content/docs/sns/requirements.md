...
| 第2段階: 応用SNS | 実務寄りの機能と運用まで広げる | メール確認、DMチャット、CI/CD |

最初から第2段階まで一気に作る必要はありません。第1段階が動けば、ポートフォリオとして最低限説明できるSNSになります。第2段階は、メール確認、リアルタイム通信、CI/CDまで伸ばし、実務に近い開発フローを経験する段階です。画像アップロード、ページネーション、通知、クラウドデプロイは発展課題として扱います。
...
共通解答コードの必須範囲では、画像アップロードを入れず、プロフィール画像は空または固定URLで構いません。S3やpresigned URLを使う画像アップロードは、NestJS詳細チュートリアルまたは各スタックの発展課題で扱います。
...
| `POST` | `/users/me/avatar-upload-url` | 要 | 発展 | アバター画像アップロードURL発行 |
...
通知テーブル、画像アップロード用テーブル、ページネーション専用のcursor設計は発展課題です。第2段階の必須範囲は、メール確認、会話、メッセージ、リアルタイムDM、CI/CDまでに固定します。
...
画像アップロード、ページネーション、Dockerイメージ作成、AWSまたは同等のクラウド環境へのデプロイ、通知機能は発展課題として扱います。NestJS詳細チュートリアルではAWS構成まで学びますが、各言語版の解答リポジトリはローカルで主要機能が動くことを基準にしています。
