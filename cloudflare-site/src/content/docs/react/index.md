...
フロントエンドのカードでは以前、「React」「フォームと一覧」「API通信」が別カードに見えていました。しかし実際には、フォーム入力、一覧表示、状態更新、API通信はすべてReact基礎の中で連続して学ぶ内容です。そのため、このセクションではReactの基礎から実用UI、API連携までを1つの流れにまとめています。
...
Reactは、HTMLを直接書き換える代わりに、**状態からUIを作る**ためのライブラリです。フォーム、一覧、API通信は別々の技術に見えますが、Reactではすべて「stateが変わると画面が変わる」という同じ考え方で扱います。

例えば、Todoアプリでは次の流れになります。

1. フォームに入力する
2. stateに新しいTodoを追加する
3. 配列を `map` で一覧表示する
4. APIから取得したTodoでstateを更新する
5. ローディングやエラーもstateで表示する

だから、React基礎では「コンポーネントだけ」「フォームだけ」「APIだけ」と分けず、1つのアプリを作れる順番で学びます。

```mermaid
flowchart LR
    A["なぜReactか<br/>仮想DOM・SPA"] --> B["環境構築<br/>Vite + React"]
    B --> C["JSXと<br/>コンポーネント"]
...
また、後半の[実践: フルスタックTodoアプリ](/fullstack-todo/)と[SNS開発（最終プロジェクト）](/sns/)は、**Reactを基準にして解説します**。Next.js、Vue.js、Nuxtでも似たアプリは作れますが、すべてのフレームワークで同じTodo/SNSを作ると学習量が膨大になります。そのため、このカリキュラムではReactで画面開発の型を固め、他のフレームワークは「役割の違い」「現場で見たときの読み方」「Reactと違う判断ポイント」を理解する位置づけにします。

つまり、ここで学ぶ内容は「半年後に自分でSNSを作る」ための土台です。一つひとつのページを、手を動かしながら確実に進めていきましょう。

## 参考リンク

- [React Learn](https://react.dev/learn) - React公式の学習ページです。コンポーネント、state、イベント、Effectの考え方を体系的に確認できます。
- [React TypeScript Cheatsheet](https://react-typescript-cheatsheet.netlify.app/) - ReactをTypeScriptで書くときの型の付け方を確認できます。
- [MDN: Client-side web APIs](https://developer.mozilla.org/en-US/docs/Learn_web_development/Extensions/Client-side_APIs) - fetchやブラウザAPIの背景を確認できます。
