...
ここで身につけた開発サイクルは、このカリキュラムの残りすべてで毎日使います。次のセクション[React基礎](/react//)からは、新しく作るプロジェクトをすべてGitで管理しながら進めてください（誰かに指示されなくても、です）。コミットの積み重ねがそのまま学習の記録になります。
