...
さらに先の[CI/CD](/cicd/)セクションでは、「GitHubへのpushやPull Requestをきっかけに、テスト・ビルドが自動で走る」仕組みをGitHub Actionsで構築します。デプロイ自動化は、その後の[AWSデプロイ](/aws/)で同じ仕組みを発展させます。また、[SNS開発](/sns/)は、GitHubリポジトリの作成から始まり、PRベースの開発フローで進める実戦そのものです。このセクションの内容が少しでも曖昧なら、今のうちに該当ページへ戻って固めておいてください。
