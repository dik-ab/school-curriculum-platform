...
- `@MaxLength(100)` — 100文字以内に制限します。[スキーマ定義](/fullstack-todo/setup/)で `@db.VarChar(100)` とした、**DB側の制約と揃えている**点が重要です。DTOで弾かなければ、DBに入らない長さの文字列がエラーを引き起こします
...
APIが完成しました。次の[フロントエンド: 画面の実装](/fullstack-todo/frontend/)では、Reactでこの APIを呼び出す画面を作ります。`fetch` によるAPI通信は[fetchでAPI通信](/react/api_fetch/)の復習です。

なお、このページで書いたService（存在確認 + 404、DTOによる検証）のパターンは、[SNS開発の投稿機能](/sns/posts/)でもそのまま登場します。
