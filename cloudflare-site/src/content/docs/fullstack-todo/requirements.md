...
このページは、Todoアプリをどのバックエンドフレームワークで実装しても変えない**共通仕様**です。Reactフロントエンド、画面、API、データモデル、エラー時の扱い、完成条件をここで固定し、各スタックのページでは「この仕様をそのフレームワークでどう作るか」だけに集中します。

解答コードは、各バックエンド版のページから確認できます。Reactフロントエンドは共通リポジトリ、バックエンドはNestJS版とSpring Boot版を公開済みです。

Todoアプリは、全言語比較の最終成果物ではなく、React、API、DBを初めて通しで接続するための小さな実践です。そのため公開済みの実装は、詳細チュートリアルがあるTypeScript / NestJS版と、Java / Spring Boot版に絞ります。Python、PHP、Go、Rubyを含む6スタック比較は、次の[SNS開発](/sns/)で扱います。
...
エラー時は、HTTPステータスで原因を判定できるようにします。フレームワークごとに標準のJSON形式は違うため、Todoアプリでは `code` のような独自エラーコードまでは共通化しません。

| 状況 | HTTPステータス | React側の扱い |
|---|---:|---|
| 入力値が不正 | `400` | 入力内容を見直すメッセージを表示する |
| 指定IDのTodoがない | `404` | 対象が見つからないメッセージを表示する |
| 想定外のサーバーエラー | `500` | API側のログを確認する |

React共通フロントエンドでは、まず `res.ok` を確認し、失敗時は `APIエラー: 400 Bad Request` のようにHTTPステータスを含むメッセージを表示します。SNS開発のように認証・権限・複数ユーザーが絡む段階では、必要に応じて独自のエラーコード設計へ進みます。
...
- 入力エラーと存在しないIDへの更新/削除で、適切なHTTPステータスを返す
...
どのバックエンドで実装するかを選ぶ場合は、[Todo NestJS + Prisma版](/fullstack-todo/nestjs/) または [Todo Spring Boot + JPA版](/fullstack-todo/spring_boot/) の個別ページへ進みます。以降のセットアップ、バックエンド実装、フロントエンド接続、動作確認は、選んだ版のページ内で進めます。PHP、Go、Rubyなど別言語で同じ思想を比較したい場合は、次の[SNS開発](/sns/)で各言語版カードを選んでください。
