...
- Goの実行方法とmodule（`go mod`）の役割を説明できる
- 変数、型、条件分岐、繰り返し、slice、mapを読める
- structとinterfaceを使った型の設計を理解できる
- `error` を明示的に返すエラー処理の流れを読める
- goroutineとchannelによる並行処理の入口を理解し、Ginに進む準備を整える
