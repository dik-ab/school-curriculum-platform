...
次のセクション[AIチャット開発（RAG）](/ai-chat//)では、立場が変わります。AIを「使う側」から、**AIを組み込んだアプリケーションを「作る側」**になり、「このカリキュラムに質問できるQ&Aボット」を開発します。そこで直面するのが、このセクションで学んだコンテキストウィンドウの限界です。カリキュラム全体のような大量の知識はコンテキストに入りきりません。この問題を「必要な部分だけ検索してコンテキストに入れる」ことで解決する技術が **RAG（検索拡張生成）** です。まずは[RAGとは何か](/ai-chat/what_is_rag/)から始めましょう。

また、ここで身につけたAIとの開発ループは、[SNS開発（最終プロジェクト）](/sns//)を進める際の強力な武器になります。設計と理解は自分で、加速はAIで——この分担を忘れずに進んでください。
