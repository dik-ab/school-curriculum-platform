...
description: Claude Codeのビルトイン・カスタムスラッシュコマンドとskillsで、よく使う指示を再利用可能な部品にする方法を学ぶ
parent: AI開発入門
nav_order: 4
---
...
`/fix-issue 123` と実行すると、ファイル中の `$ARGUMENTS` がすべて `123` に置き換わった指示がAIに送られます。`$1`、`$2` のように位置を指定して個別の引数を受け取ることもできます（`/deploy staging v2` なら `$1` が `staging`、`$2` が `v2`）。
...
これで、Claude Codeを「導入し（[導入](/ai/claude_code_setup/)）、プロジェクトに適応させ（[CLAUDE.md](/ai/claude_md/)）、定型作業を部品化する（このページ）」ところまで揃いました。次のページ[MCPで外部ツールとつなぐ](/ai/mcp/)では、Claude Codeを**コードベースの外**——Figmaのデザインやブラウザ——につなぎ、使える道具そのものを増やす方法を学びます。
