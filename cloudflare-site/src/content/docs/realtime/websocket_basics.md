...
ターミナルで作業用ディレクトリを作り、pnpmでプロジェクトを初期化して`ws`パッケージをインストールします（pnpmの導入は[実践: フルスタックTodoアプリ](/fullstack-todo/nestjs/setup/)を参照）。
...
ここで学んだハンドシェイクの知識は、[SNSのDMチャット](/sns/nestjs/chat/)で通信がうまくいかないときのデバッグ（開発者ツールでの確認）にも役立ちます。
