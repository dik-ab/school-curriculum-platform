...
- [プロジェクトセットアップ](/sns/nestjs/project_setup/) — `compose.yaml` / `backend/` / `frontend/` の骨格、ValidationPipe・CORS・PrismaModule・`GET /health`
- [ユーザー登録とログイン（JWT認証）](/sns/nestjs/auth/) — `backend/src/auth/`、`frontend/src/lib/apiClient.ts`、`useHashRoute`
- [メールアドレス確認（SES）](/sns/nestjs/email_verification/) — `backend/src/mail/`、`EmailVerificationToken`、`#/verify-email`
- [投稿機能とタイムライン](/sns/nestjs/posts/) — `backend/src/posts/`、`TimelinePage` / `PostCard` / `Layout`
- [いいね機能](/sns/nestjs/likes/) — `backend/src/likes/`（P2002→409 / P2025→404）
- [フォローとフォロー中タイムライン](/sns/nestjs/follow/) — `backend/src/users/`、`GET /posts/timeline`、`UserPage`
- [DMチャット（リアルタイム）](/sns/nestjs/chat/) — `backend/src/chat/`（REST + ChatGateway）、`ChatPage`
- [プロフィール編集と画像アップロード](/sns/nestjs/profile_and_images/) — `PATCH /users/me`、presigned URL発行、`SettingsPage`
- [SNSのテストを書く](/sns/nestjs/testing/) — `posts.service.spec.ts`（単体）、`test/follow.e2e-spec.ts`（E2E）
- [AWSへの全体デプロイ](/sns/nestjs/deploy/) — `backend/Dockerfile`、`infra/`（CDK）、`.github/workflows/`
...
- **デプロイ（`infra/` と `deploy.yml`）** — CDKスタックは `pnpm exec cdk synth` が通ることまで確認済みで、**実際のデプロイ（`cdk deploy`）は行っていません**。デプロイ手順・料金の注意は [deployの章](/sns/nestjs/deploy/) を参照してください（RDS / ALB / NAT Gateway は時間課金です。練習後は必ず `cdk destroy`）。
